...
- Sesión inexistente → `WEBPAY_SESSION_NOT_FOUND`. Una sesión persistida autorizada (`committedAt`, `responseCode === 0` y `tbStatus === AUTHORIZED`) → `ok`; un resultado persistido no autorizado → `canceled`. Ambos son idempotentes y no vuelven a llamar a Transbank.
- Las versiones anteriores usaban `committedAt` como claim antes de guardar el resultado. Una fila legada con `committedAt` pero sin respuesta de Transbank se considera `pending`, nunca autorizada.
