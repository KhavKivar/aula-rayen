# web/direct-backend-transport Specification

## Purpose
Permite al frontend TanStack Start consumir la API de cursos, pagos y gestión directamente contra `VITE_PUBLIC_API_URL` (`api.psicologarayen.cl` en producción) a través de la capa `apiClient` (axios), eliminando el helper SSR `backend-api.server.ts` y los wrappers `createServerFn`. Restaura el patrón directo previo a `01a7676` (axios `withCredentials: true`) para todos los endpoints.
...
- **THEN** its `baseURL` equals `env.VITE_PUBLIC_API_URL` (e.g., `https://api.psicologarayen.cl` in prod, `http://localhost:3000` in dev), `withCredentials` is `true`, and default header `Accept: application/json` is present
...
- **THEN** no code reads `getRequestHeader("cookie")` nor calls `setResponseHeader("Cache-Control", "no-store")` for these paths; authentication relies on `apiClient` `withCredentials: true` against the API origin with CORS credentials
...
The system SHALL resolve the API base URL from `VITE_PUBLIC_API_URL` (e.g., `https://api.psicologarayen.cl` in prod, `http://localhost:3000` in dev) via `src/config/env.ts`, and all course/Webpay/management calls SHALL be issued through `apiClient` with `withCredentials: true` so the shared session cookie is sent to the API origin without manual header manipulation.

#### Scenario: Production origin resolves to API URL
- **WHEN** the app runs in production with `VITE_PUBLIC_API_URL=https://api.psicologarayen.cl`
- **THEN** course calls target `https://api.psicologarayen.cl/courses` (and subpaths) via `apiClient` with `withCredentials: true`
