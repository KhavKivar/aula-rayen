...
- [x] 7.5 Smoke dev: `VITE_PUBLIC_API_URL=http://localhost:3000` — login, `/dashboard` catálogo, `/courses/:id` video/file, Webpay `token/url`, gestión crear/editar/eliminar. — Verificado con backend local.
- [x] 7.6 Smoke prod contra `https://api.psicologarayen.cl` — misma matriz; confirmar que `withCredentials` envía `better-auth.session_token` a la API (cookie `Secure`, `HttpOnly`, `SameSite=Lax`, `Domain=.psicologarayen.cl`). — Verificado en producción.
