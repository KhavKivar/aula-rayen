...
- Reconfigurar `src/lib/auth-client.ts` para apuntar directamente al backend Worker (`VITE_PUBLIC_API_URL` / `https://aula-rayen.vasvani.shop/api`) en lugar de `window.location.origin` / `VITE_PUBLIC_SITE_URL`. Definir `baseURL` único y habilitar `credentials: "include"` para que Better Auth envíe cookies cross-site/same-site según corresponda.
- Actualizar `src/config/env.ts`: `VITE_PUBLIC_API_URL` pasa a ser el origen canónico de la API en producción (`https://aula-rayen.vasvani.shop/api`); documentar que en local sigue siendo `http://localhost:3000` y que el proxy local ya no existe.
...
- **Infra/Deploy:** Worker Cloudflare de `apps/web` (menos código), Worker/API en `aula-rayen.vasvani.shop/api/` (ya desplegado), variables `VITE_PUBLIC_API_URL`, `VITE_PUBLIC_SITE_URL`, `BETTER_AUTH_URL`, `FRONTEND_URL`, `BETTER_AUTH_COOKIE_DOMAIN` en `.env.example` y secrets de GitHub Actions.
