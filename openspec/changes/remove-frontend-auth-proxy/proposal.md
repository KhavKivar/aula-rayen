...
- Reconfigurar `src/lib/auth-client.ts` para apuntar directamente al backend Worker (`NEXT_PUBLIC_API_URL` / `https://aula-rayen.vasvani.shop/api`) en lugar de `window.location.origin` / `NEXT_PUBLIC_SITE_URL`. Definir `baseURL` único y habilitar `credentials: "include"` para que Better Auth envíe cookies cross-site/same-site según corresponda.
- Actualizar `src/config/env.ts`: `NEXT_PUBLIC_API_URL` pasa a ser el origen canónico de la API en producción (`https://aula-rayen.vasvani.shop/api`); documentar que en local sigue siendo `http://localhost:3000` y que el proxy local ya no existe.
...
- **Infra/Deploy:** Worker Cloudflare de `apps/web` (menos código), Worker/API en `aula-rayen.vasvani.shop/api/` (ya desplegado), variables `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_SITE_URL`, `BETTER_AUTH_URL`, `FRONTEND_URL`, `BETTER_AUTH_COOKIE_DOMAIN` en `.env.example` y secrets de GitHub Actions.
- **Tests:** Eliminar/migrar `auth-proxy.test.ts`; actualizar tests de `login`, `register`, `password-recovery`, `course-dashboard`, `course-management`, `backend-api.server.test.ts` que mockean `NEXT_PUBLIC_API_URL` vs proxy.
