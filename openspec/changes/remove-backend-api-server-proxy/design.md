...
**Elegido:** Validar input con zod **antes** de `apiClient` (`courseIdSchema`, `createWebPayDtoSchema`, `createCourseRequestSchema`, `updateCourseInputSchema`, `deleteCourseInputSchema`), luego `apiClient.get/post/patch/delete` con `withCredentials` heredado. En prod `withCredentials` envía la cookie `Domain=.psicologarayen.cl` a `api.psicologarayen.cl`; en dev `localhost:3000` vs `localhost:3001` es cross-site pero `withCredentials` + CORS ya configurado lo permite. No usar `getRequestHeader`/`setResponseHeader`.  
...
7. Smoke dev (`VITE_PUBLIC_API_URL=http://localhost:3000`) y prod/staging (`https://api.psicologarayen.cl`): login, `/dashboard` catálogo, `/courses/:id`, Webpay `token/url`, y gestión (crear/editar/eliminar).
