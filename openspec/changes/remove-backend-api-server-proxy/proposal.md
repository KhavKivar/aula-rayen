...
Con la API expuesta directamente en `api.psicologarayen.cl` (CORS con credenciales y cookies cross-subdominio), el navegador puede alcanzarla sin proxy. Antes de `01a7676` el frontend ya tenía transporte directo vía `apps/web/src/lib/api-client.ts` (axios `baseURL: VITE_PUBLIC_API_URL`, `withCredentials: true`, interceptor `SessionExpiredError` para `401`). Ese archivo fue borrado en `01a7676 fix: forward sessions through server functions` y reemplazado por el proxy SSR `src/lib/backend-api.server.ts` (`requestBackendJson` con `getRequestHeader("cookie")` + `setResponseHeader("Cache-Control: no-store")` + `BackendApiError`) y wrappers `createServerFn` en `course-dashboard` (`get-courses`, `get-course`, `create-webpay`) y `course-management` (`create-course`, `update-course`, `delete-course`) para reenviar cookies en SSR. Con el transporte directo ese proxy ya no aporta valor: añade un hop, duplica lógica de fetch/validación, oculta errores y obliga a mantener `routeTree` y tests del hop. Restaurar la capa `apiClient` elimina el proxy SSR, reduce latencia y vuelve al patrón directo documentado en `AGENTS.md`, mismo movimiento ya aplicado al proxy de auth en `remove-frontend-auth-proxy`.
...
- Documentación y env: confirmar `VITE_PUBLIC_API_URL=https://api.psicologarayen.cl` en prod y `http://localhost:3000` en dev (`apps/web/.env.example`, `README.md`, `AGENTS.md` integración); no se introduce nueva variable.
...
- **Infra/Deploy:** Sin cambios de infraestructura; `api.psicologarayen.cl` ya expone la API con CORS. No requiere nuevas variables ni cambios CORS (`auth/direct-transport` ya lo cubre).
