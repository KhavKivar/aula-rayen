...
export function CourseContent({ courseId }: { courseId: number }) {
  ...
  return (
    <QueryState
      ...
      error="No fue posible abrir este curso"
      errorDescription="Verifica que tengas acceso e inténtalo nuevamente desde tu panel."
      errorAction={
        <Link
          to="/dashboard"
      ...
      loadingClassName="min-h-svh rounded-none border-0 bg-transparent"
      errorClassName="min-h-svh rounded-none border-0 bg-transparent"
    >
      {course ? (
